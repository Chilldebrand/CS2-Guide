# Ancient utility visual assets

Lineup asset names use `<map>-<side>-<grenade>-<target>-<stage>`, where `<stage>` is `start`, `aim`, or `result`.

Each lineup card must identify:

- the exact target from `maps/ancient/utility.md`;
- purpose and timing copied from the exact target row in `maps/ancient/utility.md`;
